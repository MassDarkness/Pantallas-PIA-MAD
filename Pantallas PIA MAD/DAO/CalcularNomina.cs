...
namespace Pantallas_PIA_MAD.DAO
{
    public class CalculoNomina
    {
        ...
        public Recibo_Nomina CalcularNomina(
            Empleado empleado,
            int diasTrabajados,
            decimal aguinaldoInput,
            decimal bonoPuntualidadInput,
            decimal bonoAsistenciaInput,
            decimal cuotaImssInput,
            decimal cuotaSindicalInput
        )
        {

            decimal sueldoBruto = (empleado.salario ?? 0) * diasTrabajados;

            decimal percepcionesExtras = aguinaldoInput + bonoPuntualidadInput + bonoAsistenciaInput;
            decimal totalPercepciones = sueldoBruto + percepcionesExtras;
            ...
            {
                deduccionImss = CalcularIMSS(empleado);

                decimal baseIsr = (empleado.salario ?? 0) * 30;
                deduccionIsr = CalcularISR(baseIsr);
            }

            decimal deduccionesExtras = cuotaImssInput + cuotaSindicalInput;
            decimal totalDeducciones = deduccionImss + deduccionIsr + deduccionesExtras;
        ...
        private decimal CalcularIMSS(Empleado empleado)
        {
